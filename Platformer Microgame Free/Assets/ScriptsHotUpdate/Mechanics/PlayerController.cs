...
namespace PlatformerMicrogame
{
    ...
    /// </summary>
    public class PlayerController : LikeBehaviour
    {
        ...
        public int jumpState = 0;// JumpState_Grounded;
        ...
        protected void Update()
        {
            if (controlEnabled)
            {
                ...
                if (jumpState == JumpState_Grounded && Input.GetButtonDown("Jump"))
                    jumpState = JumpState_PrepareToJump;
        ...
        {
            jump = false;
            if (jumpState == JumpState_PrepareToJump)
            {
                jumpState = JumpState_Jumping;
                jump = true;
                stopJump = false;
            }
            else if (jumpState == JumpState_Jumping)
            {
                if (!IsGrounded)
                {
                    (Simulation.Schedule(typeof(PlayerJumped)) as PlayerJumped).player = this;
                    jumpState = JumpState_InFlight;
                }
            }
            else if (jumpState == JumpState_InFlight)
            {
                if (IsGrounded)
                {
                    (Simulation.Schedule(typeof(PlayerLanded)) as PlayerLanded).player = this;
                    jumpState = JumpState_Landed;
                }
            }
            else if (jumpState == JumpState_Landed)
            {
                jumpState = JumpState_Grounded;
            }
        }
        ...
        //    Landed
        //}
        public static int JumpState_Grounded = 0;
        public static int JumpState_PrepareToJump = 1;
        public static int JumpState_Jumping = 2;
        public static int JumpState_InFlight = 3;
        public static int JumpState_Landed = 4;
    }
}