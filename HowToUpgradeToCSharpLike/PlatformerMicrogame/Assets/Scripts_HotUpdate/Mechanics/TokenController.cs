...
namespace Microgame
{
    ...
    public class TokenController : LikeBehaviour
    {
        ...
        void Start()
        {
            ...
            //Get all tokens from prefab
            if (count > 0)
            {
                tokens = new TokenInstance[count];
                for (int i = 0; i < count; i++)
                {
                    tokens[i] = HotUpdateBehaviour.GetComponentByType(GetGameObject("tokens"+i), typeof(TokenInstance)) as TokenInstance;
                }
            }
        ...
        void CheckDistance()
        {
            if (playerTranform == null)
            {
                ...
                foreach (SpriteRenderer sr in GameObject.FindObjectsOfType<SpriteRenderer>(true))
                {
