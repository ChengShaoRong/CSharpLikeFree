		...
		public void UpdateFieldInEditor(KissSerializableObject field)
		{
			...
			switch (field.objectType)
			{
				case KissSerializableObject.ObjectType.LikeBehaviourObject:
					{
						HotUpdateBehaviour hotUpdateBehaviour = field.BehaviourValue;
						object value = null;
						if (hotUpdateBehaviour != null)
						{
							value = hotUpdateBehaviour.ScriptInstance;
							if (value == null)
							{
								hotUpdateBehaviour._ForceInit();
								value = hotUpdateBehaviour.ScriptInstance;
							}
						}
						if (value != null)
				...
				case KissSerializableObject.ObjectType.LikeBehaviourObjects:
					{
						IList list = field.Value as IList;
						if (sInstance != null)
						{
				...
				case KissSerializableObject.ObjectType.LikeBehaviourObjects2:
					{
						Array list = field.Value as Array;
						...
						if (sInstance != null)
						{
							...
							{
								listInstance[i] = list.GetValue(i) as SInstance;
							}
							if (!sInstance.SetMemberValue(field.name, listInstance))
							{
								Debug.LogWarning($"Not exist member '{field.name}' in GameObject '{name}'");
						...
						else if (likeBehaviour != null)
						{
							...
							if (fi != null)
							{
								Array listLikeBehaviour = Activator.CreateInstance(fi.FieldType, new object[] { count }) as Array;
								for (int i = 0; i < count; i++)
								{
									listLikeBehaviour.SetValue(list.GetValue(i), i);
								}
								fi.SetValue(likeBehaviour, listLikeBehaviour);
							}
						}
					...
					case KissSerializableObject.ObjectType.LikeBehaviourObject:
						{
							...
							if (value != null)
							{
								...
								Debug.Log($"'{field.name}' Value null in GameObject '{name}'");
#endif
						}
						break;
					case KissSerializableObject.ObjectType.LikeBehaviourObjects:
					...
					case KissSerializableObject.ObjectType.LikeBehaviourObjects2:
						{
							...
							if (sInstance != null)
							{
								SInstance[] listInstance = new SInstance[count];
								for (int i = 0; i < count; i++)
								{
									listInstance[i] = list.GetValue(i) as SInstance;
								}
...
#region event of EventSystem
		...
		public object GetComponentEx(string bindHotUpdateClassFullName)
		{
			foreach (HotUpdateBehaviour one in gameObject.GetComponents<HotUpdateBehaviour>())
			{
				...
				{
					object obj = one.ScriptInstance;
					if (obj == null)//Try initialize
					{
						one._ForceInit();
						obj = one.ScriptInstance;
					}
					return obj;
				}
			}
